	...
	function initiateZoom() {
		...
		minZoom = Math.max($('#geomap').width() / w, $('#geomap').height() / h);
		...
		midX = ($('#geomap').width() - minZoom * w) / 2;
		midY = ($('#geomap').height() - minZoom * h) / 2;
	...
	var svg = d3.select('#geomap')
				.append('svg')
				// set to the same size as the 'geomap' div
	...
	// get map data
	d3.json('js/geomap/worldmap/world_map.json', function(json) {
		...
		countries = countriesGroup.selectAll('path')
									...
									.data(json.features.filter(d => d.properties.iso_a3 !== 'ATA' && d.properties.iso_a3 !== 'GRL'))
									.enter()
									.append('path')
									.attr('d', path)
